<template>
    <div id="app" class="chat-container-app">
        <ul class="chat-container">
            <li v-for="(message,index) in messages" :key="index" :class="(selfUserId == message.toUserId)?'friend':''">
...
<script>
  import {apiReady, getPageParams, sendEvent, toast, addEventListener, openWindow, uploadFile} from '../../utils/ApiCloudUtils'
  import {messagesBefore, messagesAfter, messagePush} from '../../utils/DataUtils'
  import {
  ...
  export default {
    ...
    methods: {
      ...
      async getBeforeMessgaes(params){
        ...
        if(c === 0){
          return d.messages
          // this.concatMessages(d.messages)
        }
      },
      //轮询获得当前最新聊天记录
      async getMessagesAfter(){
        let params = {
          friendId: this.targetUser.juid,
          pageSize:10
        }
        let endNum = this.messages.length-1
        if(endNum>0){
          params.pointMid = this.messages[endNum].mid
        }
        let {c,d} = await messagesAfter(params)
        if(c === 0){
          if(d.messages.length>0){
            for(let i=0;i<d.messages.length;i++){
              this.messages.push(d.messages[i])
              this.goBottom()
            }
          }
        }
      ...
      async sendTextMessage (params) {
        ...
        //   type:1,
        //   content:msg.trim(),
        //   toUserId:this.targetUser.juid
        // }
        params.toUserId = this.targetUser.juid
        let {c,d} = await messagePush(params)
        if(c === 0){
          this.concatMessages(d, true)
          this.getMessagesAfter()
          this.goBottom()
          // d.created = new Date().toLocaleTimeString()
      ...
      concatMessages (messages, newMessages = false) {
        ...
        if(newMessages){
          ...
          messages.forEach(item => {
            item.timeSpan = ''
            if (lastMessage !== null && Math.abs(lastMessage.created - item.created) <= 60000) {
              return
            }

            let diff = (new Date().getTime() - item.created) / 1000
            let timeSpan = diffTimestampFormat(diff, item.created)
      ...
      async getData () {
        ...
        let params = {
          friendId:this.targetUser.juid,
          pageSize:limit,
          pointMid:this.pointMid
        }
        let messages = await this.getBeforeMessgaes(params)
      ...
      // sendImageMessage ({ base64Data, data }) {
      ...
      //   })
      // },
      async sendImageMessage ({ base64Data, data }) {
        let {c,d} = await uploadFile(data)
        console.log('ccc')
        console.log(c)
        console.log(d)
        if (c === 0) {
      ...
      async openChatBox () {
        ...
        tool.open({
          ...
          tools: {
            ...
            //   normal: 'widget://images/chat/album1.png',  //字符串类型；常态下的图标，要求本地路径（fs、widget）
            //   selected: 'widget://images/chat/album2.png' //字符串类型；选中后的图标，要求本地路径（fs、widget），同按下时高亮状态公用同一个图标
            // },
            append: {
          ...
          isShowAddImg: false,
          emotionBtn: {
                          normalImg: 'widget://images/chat/album1.png'
                      } ,
      ...
      formatContent (message) {
        ...
        if (message.type === 1) {            //文字表情类型
          return message.content
          // return parseEmojiContent(message.content)
        } else if (message.type === 3) {     //图片类型
