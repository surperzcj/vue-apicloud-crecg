<template>
    <div id="app">
        <div class="progress-rollback" v-if="showRollback" @click="rollback" :style="`height: ${footerHeight}px;`">
            上一步
        </div>
        <!-- <ProjectHeader :project="project"/> -->
        <multi-selector :list="selectList" ref="selectList" title="修改状态"
                        v-on:btn-right-click="modifyProgressBySelector"/>
        <div class="project-plan-time" v-if="showPlanTime">
          <span class="plan-status">实施管理</span>
          <span class="plan-time" @click="plantime">时间安排</span>
            <!-- <img src="../../assets/images/btn-project-plantime.png" @click="plantime"> -->
        </div>
        <div class="project-plan-time" v-else>
          <span class="plan-status">跟踪状态</span>
        </div>
        <ul class="project-progress" :style="`padding-bottom: ${footerHeight}px`">
            <li :class="`project-progress-${progress.status} no-${index+1}`" v-for="(progress, index) in progresses">
                ...
                <ul class="progress-files" v-if="progress.files && progress.files.length > 0">
                    <li v-for="(file,fileIndex) in progress.files">
                ...
                <div class="progress-upload" @click="uploadImage(progress)"
                    ...
                    <span>添加图片</span>
                    <!--<input type="file" class="weui-input" @change="uploadImage">-->
                </div>
                <div class="progress-upload" @click="uploadAttachment(progress)"
                     v-show="progress.status === 2&&progress.lib === 'BID'">
                    <span>添加附件</span>
                    <!--<input type="file" class="weui-input" @change="uploadAttachment">-->
                </div>
